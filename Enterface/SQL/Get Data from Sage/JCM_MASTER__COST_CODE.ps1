...
$sqlcmd = "select c.Cost_Code CostCode,c.Description CostCodeDescription,1 CompanyID,c.Job JobNumber,c.Extra Extra,'' SOURCE_KEY from JCM_MASTER__COST_CODE c, JCM_MASTER__JOB j where j.job = c.job and j.Status <> 'Closed'"
...
$results4 | Export-Csv -Path C:\temp\Sage-to-csv\JCM_MASTER__COST_CODE.csv -NoTypeInformation -Verbose